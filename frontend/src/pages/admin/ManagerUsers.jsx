        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-black/10 bg-[#fafafa] p-4">
            <div>
              ...
              <p className="mt-1 text-sm text-black/60">Use a grouped form so creation feels structured and professional.</p>
            </div>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-black/10">
              <table className="min-w-full divide-y divide-black/10 text-sm">
                ...
                <tbody className="divide-y divide-black/5">
                  {loading ? <tr><td colSpan={5} className="px-4 py-4 text-black/50">Loading...</td></tr> : filteredRows.length ? filteredRows.map((row) => <tr key={row.id}><td className="px-4 py-3"><div className="flex items-center gap-3">{row.profile_picture_url ? <img src={row.profile_picture_url} alt={`${row.first_name || ''} ${row.last_name || ''}`.trim() || 'User'} className="size-10 rounded-full border border-black/10 object-cover" /> : <div className="flex size-10 items-center justify-center rounded-full border border-black/10 bg-[#f5f6fa] text-xs font-semibold text-black/60">{`${row.first_name || ''}${row.last_name || ''}`.trim().slice(0, 1).toUpperCase() || 'U'}</div>}<div>{row.first_name} {row.last_name || ''}<div className="text-xs text-black/50">{row.email}</div></div></div></td><td className="px-4 py-3">{row.designation?.name || '-'}</td><td className="px-4 py-3">{row.assignment?.location?.metadata?.name || '-'}</td><td className="px-4 py-3">{row.assignment?.department?.metadata?.name || '-'}</td><td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => handleEdit(row)} className="rounded-lg border border-black/15 px-3 py-1 text-xs">Edit</button><button onClick={() => openTransfer(row)} className="rounded-lg border border-blue-200 px-3 py-1 text-xs text-blue-700">Transfer</button><button onClick={() => handleDelete(row.id)} className="rounded-lg border border-rose-200 px-3 py-1 text-xs text-rose-700">Delete</button></div></td></tr>) : <tr><td colSpan={5} className="px-4 py-4 text-black/50">No manager users found.</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
