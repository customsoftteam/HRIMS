...
function AdminEmployeeUsersPage() {
  ...
  const fetchRows = async () => {
    const response = await fetch(`${API_BASE_URL}/api/admin/users?role=employee`, { headers: authHeaders })
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-black/10">
              <table className="min-w-full divide-y divide-black/10 text-sm">
                ...
                <tbody className="divide-y divide-black/5">
                  {loading ? <tr><td colSpan={4} className="px-4 py-4 text-black/50">Loading...</td></tr> : filteredRows.length ? filteredRows.map((row) => <tr key={row.id}><td className="px-4 py-3">{row.first_name} {row.last_name || ''}<div className="text-xs text-black/50">{row.email}</div></td><td className="px-4 py-3">{row.assignment?.location?.metadata?.name || '-'}</td><td className="px-4 py-3">{row.assignment?.department?.metadata?.name || '-'}</td><td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => handleEdit(row)} className="rounded-lg border border-black/15 px-3 py-1 text-xs">Edit</button><button onClick={() => openTransfer(row)} className="rounded-lg border border-blue-200 px-3 py-1 text-xs text-blue-700">Transfer</button><button onClick={() => handleDelete(row.id)} className="rounded-lg border border-rose-200 px-3 py-1 text-xs text-rose-700">Delete</button></div></td></tr>) : <tr><td colSpan={4} className="px-4 py-4 text-black/50">No employee users found.</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
